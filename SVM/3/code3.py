...
kernel = "poly"
trained_netwoek = svm.SVC(kernel=kernel)

models = cross_validate(trained_netwoek, x_train, y_train.ravel(), cv=5,  return_estimator=True,  n_jobs=5)

scores_of_folds = [ model.score(x_test,y_test.ravel()) for model in models['estimator'] ]

print("Maximum score" + str(max(scores_of_folds)*10))
print("Avarage score" + str(np.average(scores_of_folds)*10))
print("kernel = ", kernel)